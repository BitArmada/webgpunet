...
const renderShaderCode = await loadFile('./render.wgsl');
...
for(var i = 0; i < inputKeys.length; i++){
    ...
    input[key] = {
        x: Math.floor((i / inputKeys.length) * GRID_SIZE),
        y: 10,
        value: 0
    }
}
...
canvas.addEventListener('mousedown', async (event)=>{
    ...
    await activate(Math.floor((x/canvas.width)*GRID_SIZE), Math.floor((y/canvas.height)*GRID_SIZE), 1)
})
